...
Emits the raw JSON5 block for one template as a JSON string to **stdout**, exit
0. Exit 1 if the template id is not found.
...
```jsonc
{ "kind": "field_ref", "syntax": "[ColName]", "description": "Reference a CSV column by name." }
```
