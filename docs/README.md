...
| [gui.md](gui.md)                       | bxp-gui Flutter app: source layout, subprocess wiring, agent control (gui-mcp), auto-updater + minisign verification, json5_ast AST library, TraceStore state management, dev-run workflow, key patterns. |
| [mcp.md](mcp.md)                       | bxp-mcp MCP server (the stdio config-authoring server — distinct from the in-GUI gui-mcp): adapter model over the shared `inspect` core, tool catalog, in-proc vs spawn, wire protocol (isError/structuredContent/progress), bxp_simulate. |
