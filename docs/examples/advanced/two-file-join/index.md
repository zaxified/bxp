...
[:material-github: View on GitHub](https://github.com/zaxified/bxp/tree/master/docs/examples/advanced/two-file-join){ .md-button }
...
1. **Stack the two files with a `_type` marker.** Here it ships ready-made; to
   ...
   [../multi-stage-etl](../multi-stage-etl/index.md), which builds the same
...
=== "sample.json (config)"
    ...
    ```js
    --8<-- "examples/advanced/two-file-join/sample.json"
    ```
