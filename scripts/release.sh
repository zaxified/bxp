...
#   bash scripts/release.sh v1.2.0    — uses the given version string
#
# Output: releases/bxp-cli-<version>-<platform>.(tar.gz|zip)
...
build() {
    ...
    local base="$OUTDIR/bxp-cli-${VERSION}-${name}"
    local stage
    stage=$(mktemp -d)
    mkdir -p "$stage/bxp"
    cp "$bin"                                                          "$stage/bxp/bxp-cli${ext}"
    cp "$MONO_ROOT/resources/bxp-cli.examples.json"                    "$stage/bxp/bxp-cli.examples.json"
