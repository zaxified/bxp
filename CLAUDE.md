...
│   │   ├── tz.zig          # IANA UTC-offset lookup (find/offsetAt; POSIX-footer
│   │   │                   # fallback) over generated tz_data.zig — behind the TZ builtins
│   │   ├── tz_data.zig     # GENERATED (tools/tz-gen) pinned-tzdata offset tables; committed
...
│   └── build.zig.zon     # one fetch dep: uucode (Unicode tables); date/decimal/tz cores in-house
...
│   │                         # (real-world + teaching tiers); hand-authored
│   │                         # index.md pages — see docs/examples/CLAUDE.md.
│   │                         # Indexes generated by scripts/gen-examples-index.py.
...
(`datefmt.zig`), numeric core (`decimal.zig`), and timezone core
(`tz.zig` + the generated `tz_data.zig`) remain fully in-house — the
former `sunrise` datetime dependency was replaced by `datefmt.zig`, and
full IANA/DST timezone support (the `TO_UTC`/`TZ_OFFSET`/`TZ_CONVERT`/`IS_DST`
builtins) ships as committed offset tables generated by `tools/tz-gen` from a
pinned tzdata release — no runtime dependency, `std.Tz` is used only inside the
throwaway build-time generator.
