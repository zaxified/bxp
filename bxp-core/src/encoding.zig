...
//! transcoding (the "iconv" job). No `uucode` dependency — just 256-entry
//! mapping tables for the European single-byte code pages. Internal currency
//! everywhere else in the pipeline is always UTF-8; this module only runs at
//! the CSV read edge (decode → UTF-8) and the CSV write edge (encode ← UTF-8),
//! driven by the per-template `csv_input_encoding` / `csv_output_encoding`
//! config keys.
//!
//! CSV only by design: JSON (RFC 8259) and xlsx (XML-in-ZIP) are always UTF-8,
//! so they never reach here.
//!
//! Offset-safe: in every code page below the structural CSV bytes — the field
//! delimiter, quote characters, CR and LF — are ASCII (< 0x80) and map to
//! themselves. The pipeline therefore splits records / counts byte offsets on
//! the *raw* bytes and only transcodes individual field values and header
//! names, so `source_locator` byte offsets stay correct on the original file.
//!
//! Data-lenient, matching unicode.zig: a byte / sequence that cannot be
//! transcoded is emitted verbatim (decode) or replaced with '?' (encode) —
//! never an error, never a crash.

const std = @import("std");

/// Supported text encodings for CSV input / output. `utf8` is the default and
/// a pure pass-through. The rest are the European single-byte code pages a
/// legacy broker / Excel export is realistically saved in.
pub const Encoding = enum {
    ...
    /// the config loader can emit a did-you-mean style warning.
    ...
    /// Canonical config string for this encoding — the value shown in the GUI
    /// dropdown and stored back on Save. Kept in sync with `csv_encoding_values`
    /// in config.zig (the FieldDoc enum list driving that dropdown).
