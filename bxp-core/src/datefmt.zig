//! Self-contained date/time toolkit for bxp — parsing, formatting, and
//! calendar arithmetic over a token-based format vocabulary.
//!
//! Replaces the former `sunrise` dependency (bxp-core's only external dep).
//! Everything here is built on Howard Hinnant's civil-from-days algorithm
//! (`i64`, `@divFloor`), so the whole module is correct for dates before the
//! Unix epoch — unlike sunrise, whose `std.time.epoch.EpochSeconds` (`u64`)
//! core silently floored any pre-1970 date to 1970-01-01.
//!
//! Two layers:
//!   1. Civil core — `ymdToEpochDay` / `epochDayToYmd` / `isoWeekday`,
//!      leap-year + days-in-month, name tables. Shared by the DSL date
//!      builtins (YEAR / MONTH / DATEADD / DATEDIFF / WORKDAY / EOMONTH / …).
//!   2. Token I/O — `parse` (string + format → `DateParts`) and `format`
//!      (`DateParts` + format → string). DATE_CONVERT is a pure reshuffle
//!      through these two; it never round-trips through an epoch, so it has
//!      no lower-year limit.
//!
...
//!   ss/s second · A/a AM-PM (upper/lower) · EEEE full day name
//!   EEE/EE/E short day name · e day-of-week 1-7 (Mon=1)
...
/// Calendar date + wall-clock time. Time fields default to 0; the DSL
/// date-only builtins ignore them. `year` is signed so pre-epoch years
/// round-trip cleanly through the civil core.
...
/// the per-row DATE_CONVERT path (the reason sunrise needed a vendor patch).
const MAX_TOKENS = 64;
...
/// epoch timestamp, so DATE_CONVERT round-trips any year losslessly.
...
// ---------------------------------------------------------------------------
// Strict ISO helpers (used by the DSL date builtins)
// ---------------------------------------------------------------------------

/// Parse a strict canonical `YYYY-MM-DD` date (time defaults to 00:00:00).
/// Stricter than `parse` — used where the DSL has already normalised input
/// to ISO and wants a fast, unambiguous read.
...
/// first offender, or null when every letter is a valid token. Lets the editor
