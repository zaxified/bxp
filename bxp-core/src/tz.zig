//! IANA time-zone offset lookup over the generated `tz_data.zig` tables.
//!
//! `tz_data.zig` (built by tools/tz-gen from a pinned IANA tzdata release) holds
//! each zone's UTC-offset transitions from 1970 onward plus its POSIX-TZ footer
//! rule. This module resolves a zone by name and reports the UTC offset (and DST
//! flag) at a given instant: a binary search over the explicit transitions, then
//! the POSIX rule for instants past the last one (~2038+). File-relative
//! `@import` by expr.zig — not a named module.
