...
pub fn preprocessAnnotated(alloc: std.mem.Allocator, input: []const u8) !AnnotatedResult {
    ...
    while (i < input.len) {
        ...
        // Phase 5d: pending comments are now silently dropped at boundaries.
        // The pre-flush + span-tracking hooks that the original CST patcher
        // relied on are gone; the AST library reparses raw bytes directly.
        ...
        // ── comments → strip silently (Phase 5d) ─────────────────────────
        if (c == '/' and i + 1 < input.len) {
            if (input[i + 1] == '/') {
                i += 2;
                while (i < input.len and input[i] != '\n') i += 1;
...
test "annotated: comments are silently stripped (Phase 5d)" {
    const alloc = std.testing.allocator;
