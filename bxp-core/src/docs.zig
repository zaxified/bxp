//! Aggregates and serializes the documentation surface `inspect.docsJson`
//! exposes to the GUI, and renders the Markdown reference tables.
//!
//! Two sources feed it:
//!
...
fn writeSchemaRow(w: *std.Io.Writer, full_key: []const u8, f: FieldDoc) !void {
    ...
    // The allowed values belong on the row that describes the field, not in a
    // list retyped somewhere in the guide. Appended to the description rather
    // than given a column of their own: only a handful of fields are enums, so
    // a sixth column would be empty on most rows and would push this already
    // wide table into a horizontal scroll.
    if (f.enum_values) |vs| {
        try w.writeAll(" One of: ");
        for (vs, 0..) |v, i| {
            if (i > 0) try w.writeAll(", ");
            try writeTagged(w, "hl-str", v);
        }
        try w.writeByte('.');
    }
    try w.writeAll(" |\n");
