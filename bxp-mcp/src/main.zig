...
// agent-callable tools: bxp_validate / bxp_eval / bxp_eval_batch / bxp_docs /
// bxp_list_templates / bxp_fetch_template (stateless, in-process via bxp-core's
// `inspect` module) plus bxp_simulate (a full conversion — spawns the
// co-located bxp-cli; see sim.zig).
//
    ...
    if (args.next()) |arg| {
        if (std.mem.eql(u8, arg, "--help") or std.mem.eql(u8, arg, "-h")) {
            const msg =
                ...
                \\bxp_eval_batch, bxp_docs, bxp_list_templates, bxp_fetch_template,
                \\bxp_simulate. Register with an MCP client via "command": "bxp-mcp".
                \\
            ;
